...
deploy_app() {
    ...
    # 下载jar包
    if ! wget -q https://github.com/Thuninisan/ani-rss/releases/latest/download/ani-rss.jar -O "$INSTALL_DIR/ani-rss.jar"; then
    ...
    echo "正在下载 run.sh"
    # 下载启动脚本
    if ! wget -q https://github.com/Thuninisan/ani-rss/raw/master/docker/run.sh -O "$INSTALL_DIR/run.sh"; then
    ...
    # 下载管理脚本
    if ! wget -q https://github.com/Thuninisan/ani-rss/raw/master/linux/ani-rss.sh -O "/usr/local/bin/ani-rss"; then
        echo -e "${RED}下载启动脚本失败${NC}"
        exit 1
    fi
